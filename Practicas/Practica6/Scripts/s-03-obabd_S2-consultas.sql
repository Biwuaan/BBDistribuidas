...
begin
	dbms_output.put_line('Realizando consulta empleando ligas');
		...
		from (
			select pais_id
				from F_OBA_PAIS_1@obabd_s1.fi.unam
			union all
			select pais_id
				from F_OBA_PAIS_2
		) q1;
	...
	from (
		select oficina_id
			from F_OBA_OFICINA_1@obabd_s1.fi.unam
		union all
		select oficina_id
			from F_OBA_OFICINA_2
		) q2;

	select count(*)	 as into v_num_proyectos
	from (
		select proyecto_id
			from F_OBA_PROYECTO_1@obabd_s1.fi.unam
		union all
		select proyecto_id
			from F_OBA_PROYECTO_2
		union all 
		select proyecto_id
			from F_OBA_PROYECTO_3
		) q3;

	select count(*) as into v_num_empleados
	from (
		select empleado_id
			from F_OBA_EMPLEADO_2@obabd_s1.fi.unam
		union all
		select empleado_id
			from F_OBA_EMPLEADO_3 
		) q4;

	select count(*) as into v_num_pago_empleados
	from (
		select pago_empleado_id
			from F_OBA_PAGO_EMPLEADO_1@obabd_s1.fi.unam
		union all
		select pago_empleado_id
			from F_OBA_PAGO_EMPLEADO_2
		) q5;
